...
export type ContentElem = SectionLinkElem | TextElem | MathElem | CitationElem | UnknownElem;
...
export function getAuthors(ch: CheerioAPI) {
    ...
    const authorArray = sanitizeText(authorSelect.text())
        .split(', '); // Split the authors into an array
    return authorArray
}
...
export function getKeywords(ch: CheerioAPI) {
    const keywordsText: string = sanitizeText(ch('.ltx_keywords').text().replace(/Index Terms:\s*/, ''));
...
export function parseParagraph(ch: CheerioAPI, p: Element): Paragraph {
    ...
    contentNodes.map((el) => {
        ...
        switch (tagName) {
            ...
            case 'cite':
                // @ts-ignore
                parseCitation(ch, el).map((c) => {contents.push(c)});
                break;
            case 'a':
                // @ts-ignore
...
export function parseCitation(ch: CheerioAPI, citation: Element): CitationElem[] {
    // @ts-ignore
    const chCite = ch(citation).children('a.ltx_ref').map((_, el) => {
        const chel = ch(el);
...
export function parseTitle(ch: CheerioAPI, title: Element): Title | undefined {
    ...
    for (const span of chSpans) {
        ...
        if (chSpan.hasClass('ltx_tag')) {
            ...
            classNames = [...classNames, ...(chSpan.attr('class')?.split(/\s+/) || [])];
        }
    }
    if (titleText === '') {
...
export function parseContainer(ch: CheerioAPI, container: Element): SectionContent {
    // @ts-ignore
    ...
    // @ts-ignore
    const title = titleTags.length === 0 ? undefined : parseTitle(ch, titleTags.get()[0]);
...
export function parseEquation(ch: CheerioAPI, equation: Element): Equation {
    ...
    const latexStrings = chel.find('math').map((_, el) => {
...
export function parseElements(ch: CheerioAPI, elements: Element[]): SectionContent[] {
...
export function parseMainSection(ch: CheerioAPI): Section {
    ...
    // @ts-ignore
    let sectionTags: Element[] = article
        .children('section.ltx_section').get();
