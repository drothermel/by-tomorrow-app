...
const extractTextSchema = () =>
	z
		...
		.transform((data) => data._text.replace(/\s+/g, ' ').trim())
...
/** Arxiv Query Schemas */
export const arxivSortByEnum = z.enum([
	'relevance',
	'lastUpdatedDate',
	'submittedDate',
])
...
export const arxivAuthorFieldSchema = z
	.object({
		name: extractTextSchema(),
	})
	.passthrough()
	.transform((data) => data.name)
export const arxivCategoryFieldSchema = z
	.object({
		_attributes: z.object({ term: z.string() }),
	})
	.passthrough()
	.transform((data) => data._attributes.term)
export const arxivLinkFieldSchema = z
	.object({
...
export const arxivEntrySchema = z
	.object({
		id: extractTextSchema(),
		updated: extractTextSchema(),
		published: extractTextSchema(),
		title: extractTextSchema().transform((data) => {
			return titleCase(data)
		}),
		summary: extractTextSchema(),
		author: normalizeToArray(
			z.union([arxivAuthorFieldSchema, z.array(arxivAuthorFieldSchema)])
		),
		link: z.array(arxivLinkFieldSchema),
		category: normalizeToArray(
			z.union([arxivCategoryFieldSchema, z.array(arxivCategoryFieldSchema)])
		),
...
export const arxivResponseSchema = z
	.object({
		feed: z.object({
