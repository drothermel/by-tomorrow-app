...
import { paperApiBulkAddDataSchema, paperApiRemoveDataSchema, paperApiSearchParamsSchema } from "$lib/schemas.js";
...
export async function GET({ url }) {
    ...
    if (!params.success) {
        return json({success: false, error: "Invalid query parameters", errors: params.error.errors}, {status: 400});
    }
    ...
    if (params.data.metadata) {
        if (params.data.arxivID) {
            ...
            if (paperMetadata) {
                return json({success: true, queryArxivId: queryId, metadata: paperMetadata});
            }
            return json({success: false, error: "Paper not found", queryArxivId: queryId}, {status: 404});
        }
        const allMetadata = await db.paperMetadata.findMany();
        return json({success: true, allMetadata: allMetadata});
    }
    return json({success: false, error: "Full papers not available now"}, {status: 404});
}
...
async function addPaperMetadata(data: unknown) {
    ...
    if (!validatedData.success) {
        return json({success: false, error: "Invalid data", errors: validatedData.error.errors}, {status: 400});
    }
...
async function removePaperMetadata(data: unknown) {
    ...
    if (!uniqueValData.success) {
        return json({success: false, error: "Invalid data", errors: uniqueValData.error.errors}, {status: 400});
    }
    ...
    try {
        ...
        console.error(error)
        return json({success: false, error: "Failed to delete records", errors: error.message}, {status: 500});
    }
    return json({success: true})
}